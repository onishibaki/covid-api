...
const calcDisplay = (covidDetail,countryParam) => {
    const filterCountry = covidDetail.filter((covidDetails)=> covidDetails.country == countryParam);

    const TotalPositive = filterCountry.reduce((total, filterCountrys) => filterCountrys.covid_positive + total,0);

    const TotalDeath = filterCountry.reduce((total, filterCountrys) => filterCountrys.covid_death + total,0);

    const TotalRecovered = filterCountry.reduce((total, filterCountrys) => filterCountrys.covid_recovered + total,0);

    const checkByLatest = filterCountry.filter((filterCountrys) => filterCountrys.covid_date == moment().tz('Asia/Tokyo').format('L'));
   ...
   checkByLatest.forEach((checkByLatests)=> {
    const checkExist = o => o.country == checkByLatests.country;
